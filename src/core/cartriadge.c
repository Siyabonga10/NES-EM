...
int load_cartridge_from_memory(unsigned char *data, int len, Cartriadge *cart)
{
    ...
    if (offset + pgRomSize * 0x4000 + chrRomSize * 0x2000 > len)
    {
    ...
    memcpy(cart->pg_rom, data + offset, pgRomSize * 0x4000);
    offset += pgRomSize * 0x4000;

    if (chrRomSize > 0)
    {
        memcpy(cart->ch_rom, data + offset, chrRomSize * 0x2000);
        offset += chrRomSize * 0x2000;
    }
