...
static unsigned char M000_CPU_READ(Cartriadge *cart, int addr) {
  if (addr < 0x8000) {
    if (cart->prg_ram)
...
static void M000_CPU_WRITE(Cartriadge *cart, int addr, unsigned char value) {
  if (addr < 0x8000) {
    if (cart->prg_ram)
...
void mount_mapper_000_to_cartridge(Cartriadge *cart, iNesOneRomInfo cart_info) {
  cart->cpu_read          = M000_CPU_READ;
  cart->ppu_read          = M000_PPU;
  cart->cart_writer       = M000_CPU_WRITE;
