...
static unsigned char M001_CPU_READ(Cartriadge *cart, int addr) {
    ...
    case 0x01:
        ...
        return cart->pg_rom[((0x8000 * prg_bank_no) + addr) % cart->pg_rom_size];
    case 0x02:
        if (addr < 0x4000)
            return cart->pg_rom[addr % cart->pg_rom_size];
        prg_bank_no = 0b1111 & prg_bank;
        return cart->pg_rom[((0x4000 * prg_bank_no) + (addr - 0x4000)) % cart->pg_rom_size];
    case 0x03:
        if (addr >= 0x4000)
            return cart->pg_rom[((cart->pg_rom_bank_count - 1) * 0x4000 + (addr - 0x4000)) % cart->pg_rom_size];
        prg_bank_no = 0b1111 & prg_bank;
        return cart->pg_rom[((0x4000 * prg_bank_no) + addr) % cart->pg_rom_size];
    default:
        break;
    }
    return 0;
